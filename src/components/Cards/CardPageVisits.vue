<template>
  <div>
          ...
          <table class="items-center w-full bg-transparent border-collapse">
            ...
            <tbody>
              <tr :key="index" v-for="(user, index) in paginatedData">
                ...
                <td
                  class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-no-wrap p-4 max-w-200-px"
                >
                  <input
                    v-on:keyup.enter="postFeedbackComment(user)"
                    type="text"
                    name="name"
                    class="mx-input"
    ...
    <div
      v-if="feedbackModal"
      id="overlay"
      class="relative md:ml-56"
      v-on:click="closeModal()"
    >
      <div
        class="flex items-center justify-center fixed bottom-0 w-full h-full"
      >
...
export default {
  ...
  methods: {
    ...
    async postFeedbackComment(user) {
      this.$LIPS(true);
      try {
        await post(this.apiUrls.postComment + user.id, {
    ...
    openModal(data) {
      ...
      this.customerFeedback = data.name;
